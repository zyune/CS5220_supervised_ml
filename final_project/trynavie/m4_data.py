...
def m4_parser(dataset_name, directory, num_obs=1000000):
    ...
    m4_info = pd.read_csv(data_directory+'/M4-info.csv',
                          usecols=['M4id', 'category'])
    m4_info = m4_info[m4_info['M4id'].str.startswith(
        dataset_name[0])].reset_index(drop=True)
    ...
    train_df = pd.wide_to_long(
        train_df, stubnames=["V"], i="unique_id", j="ds").reset_index()
    ...
    test_df = pd.wide_to_long(
        test_df, stubnames=["V"], i="unique_id", j="ds").reset_index()
    ...
    X_test_df = df[df['split'] == 'test'].filter(
        items=['unique_id', 'ds', 'x'])
    y_test_df = df[df['split'] == 'test'].filter(
        items=['unique_id', 'ds', 'y'])
...
def naive2_predictions(dataset_name, directory, num_obs, y_train_df=None, y_test_df=None):
    ...
    if (y_train_df is None) or (y_test_df is None):
        _, y_train_df, _, y_test_df = m4_parser(
    ...
    for unique_id in unique_ids:
        ...
        top_row = np.asscalar(
            y_train_df['unique_id'].searchsorted(unique_id, 'left'))
        bottom_row = np.asscalar(
            y_train_df['unique_id'].searchsorted(unique_id, 'right'))
        ...
        y_naive2['y_hat'] = Naive2(seasonality).fit(
            y_id.y.to_numpy()).predict(output_size)
        y_naive2_df = y_naive2_df.append(y_naive2)

    y_naive2_df = y_test_df.merge(
        y_naive2_df, on=['unique_id', 'ds'], how='left')
    y_naive2_df.rename(columns={'y_hat': 'y_hat_naive2'}, inplace=True)

    results_dir = directory + '/results'
    naive2_file = results_dir + \
        '/{}-naive2predictions_{}.csv'.format(dataset_name, num_obs)
...
def prepare_m4_data(dataset_name, directory, num_obs):
    ...
    monthlytrain_filename = maybe_download(
        'Train/Monthly-train.csv', directory)
    quarterlytrain_filename = maybe_download(
        'Train/Quarterly-train.csv', directory)
    ...
    quarterlytest_filename = maybe_download(
        'Test/Quarterly-test.csv', directory)
    ...
    if not os.path.exists(naive2_file):
        y_naive2_df = naive2_predictions(
            dataset_name, directory, num_obs, y_train_df, y_test_df)
    else:
